...
from Gophormer.proj.data.n_data_loader import SampleNodeSubgraphLoader
...
class SampleNodeTrainer():
    def __init__(self, model, g, cf, features, sup, loss_func=th.nn.CrossEntropyLoss()):
        ...
        # ! Get dataloader
        train_data = SampleNodeSubgraphLoader(g_sample, g_compute, cf, is_full=False)
        val_data = SampleNodeSubgraphLoader(g_sample, g_compute, cf, is_full=cf.val_full_inf, n_samples=cf.val_n_samples)
    ...
    def run(self):
        ...
        for epoch in range(self.epochs):
            ...
            val_dict = {'ValAcc': round(val_acc, 4)} if is_eval else {}
            self.logger.dynamic_log({'Epoch': epoch, 'Time': time() - t0, 'cla_loss': round(cla_loss, 4), 'cr_loss': round(cr_loss, 4), 'loss': round(loss, 4), 'TrainAcc': round(train_acc, 4),
                                     'LR': f'{self.optimizer.param_groups[0]["lr"]:.2e}', **val_dict}, 'INFO')
            ...
            if self.scheduler != None and self.scheduler_type == 'DecayAtPlateau':
                self.scheduler.step(metrics=val_acc)
                self.log(f'Epoch {epoch} learning rate: {self.optimizer.param_groups[0]["lr"]:.2e})', 'DYNAMIC')
    ...
    def eval_and_save(self):
        ...
        res_dict = {"res_file": self.cf.res_file, 'best_epoch': best_epoch, **result}
        self.logger.dynamic_log(result)
        self.logger.log(f'\nTrain seed{self.cf.seed} finished\nResults: {res_dict}\n{self.cf}')
        self.logger.static_log(self.cf.model_conf)
        self.logger.static_log(res_dict)
        self.logger.save()

    def process_batch(self, loader, batch_data):
        return loader.dataset.dataset.get_batch_data(batch_data)
...
def collate_fn():
    def batcher_dev(batch):
        node_ids, neighbors = zip(*batch)
        return th.stack(node_ids), neighbors
